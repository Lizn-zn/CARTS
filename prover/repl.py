...
class LeanIO():
    def __init__(self, math_dir="./math", repl_dir="./repl", force_rebuild=False, logger=None):
        ...
        print(f'Working at directory path = {current_dir}')
        ## build Math
        ...
        if (not os.path.isdir(lake_dir)) or force_rebuild:
            ...
            print(f"build mathlib: {msg}")
            msg = run_lake_build(self.MathDir, 'smt')
            print(f"build smt: {msg}")
            msg = run_lake_build(self.MathDir, '')
            print(f"build lean: {msg}")
        ## build REPL
...
class LeanIOContext:
    def __init__(self, math_dir="./math", repl_dir="./repl", force_rebuild=False, logger=None):
        self.repl = LeanIO(math_dir, repl_dir, force_rebuild, logger)

    def __enter__(self, theorem, timeout, additional_imports):
        for imp in additional_imports:
            state = self.repl.build(f"import {imp}")
        proof_state = self.repl.build(theorem)
        return self.repl, proof_state
